      ...
      for (let fi = 0; fi < nFatt; fi++) {
        ...
        h += `<tr class="${!isFirst?'fatt-subrow':''}">
          ...
          <td data-label="" class="fatt-actions">
            ${isLast ? `<button class="btn-add-fatt" onclick="addFattura(${m})" title="Aggiungi" aria-label="Aggiungi">+</button>` : ''}
            <button class="btn-del-fatt" onclick="removeFattura(${m},${fi})" title="Rimuovi" aria-label="Rimuovi">&times;</button>
          </td></tr>`;
      }
...
function renderBudget() {
  ...
  for (let i = 0; i < data.budget.length; i++) {
    ...
    h += `<div class="budget-row budget-row-5">
      ...
      <button class="btn-del" onclick="data.budget.splice(${i},1);saveData();renderBudget()" title="Elimina budget" aria-label="Elimina budget">&times;</button>
    </div>`;
  }
...
function renderSpese() {
  ...
  for (let i = 0; i < data.spese.length; i++) {
    ...
    h += `<div class="spese-row">
      ...
      <button class="btn-del" onclick="data.spese.splice(${i},1);saveData();recalcAll()" title="Elimina spesa" aria-label="Elimina spesa">&times;</button>
    </div>`;
  }
